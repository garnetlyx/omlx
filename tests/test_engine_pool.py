...
class TestEnginePoolInUseLease:
    ...
    async def test_release_engine_unloads_pending_after_lease_drains(self):
        ...
        pool._unload_engine.assert_awaited_once_with("leased", reason="hard memory pressure", source=None)
    ...
    async def test_manual_unload_waits_for_scheduler_quiescence(self):
        ...
        unloaded = await pool.request_unload(
            "leased", source="path=/api/models/leased/unload"
        )

        assert unloaded is False
        assert entry.pending_unload_reason == "manual unload"
        assert entry.pending_unload_source == "path=/api/models/leased/unload"
        ...
        pool._unload_engine.assert_awaited_once_with(
            "leased",
            reason="manual unload",
            source="path=/api/models/leased/unload",
        )
        assert entry.pending_unload_reason is None
        assert entry.pending_unload_source is None
    ...
    async def test_manual_unload_stops_idle_engine_immediately(self):
        ...
        pool._unload_engine.assert_awaited_once_with("idle", reason="manual unload", source=None)
